...
/// <summary>
/// Plays back a capture file recorded by the R3eCaptureTool as if it were the live shared memory, so the
/// bridge can be developed and tested without the game running. The capture stores one buffer stream per
/// index (matching <see cref="R3eBufferType"/> - full $R3E snapshots plus a small high-rate probe), keyed by
/// capture time.
/// </summary>
public class R3eCapturePlayer : R3eDataProvider
{
	...
	public R3eCapturePlayer( string captureFilePath )
	{
	...
	public override bool TryOpen()
	{
		...
		if ( magic != FileMagic )
		{
			...
			throw new InvalidDataException( $"{_captureFilePath} is not an R3E capture file." );
		}
