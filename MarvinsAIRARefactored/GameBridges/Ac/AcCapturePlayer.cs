...
/// <summary>
/// Plays back a capture file recorded by the AcCaptureTool as if it were the live shared memory, so the
/// bridge can be developed and tested without the game running. The capture stores one buffer stream per AC
/// page (indices matching <see cref="AcBufferType"/>), keyed by capture time.
/// </summary>
public class AcCapturePlayer : AcDataProvider
{
	...
	public AcCapturePlayer( string captureFilePath )
	{
	...
	public override bool TryOpen()
	{
		...
		if ( magic != FileMagic )
		{
			...
			throw new InvalidDataException( $"{_captureFilePath} is not an AC capture file." );
		}
